...
def ftx_price(client, symbol, side="ask"):
    ...
    res = float(
        markets[
            (markets["baseCurrency"] == symbol)
            & (markets["quoteCurrency"] == source_symbol)
        ][side]
    )
    time.sleep(0.05)
    return res
...
def place_trigger_order(
    client, symbol, buy_price, size, trigger_name, trigger_param, round_n=4, debug=False
):
    size = floor_to_n(size, round_n)
...
def modify_trigger_order(
    client, order_id, trigger_name, size, trail_value=None, round_n=4, debug=False
):
...
def get_weight_diffs(client, buy_info, symbols):
    ...
    total_balance, balances = get_total_balance(
        client, separate=True, filter=list(target_weights.keys())
    )

    missing_symbols = [k for k in target_weights.keys() if k not in balances.index]
    missing_balances = pd.DataFrame(
        index=missing_symbols, columns=balances.columns
    ).fillna(0)
...
def transfer_taxes_to_taxes_subaccount(client, size, debug=False):
    if not debug:
        client.transfer_to_subaccount(
            coin=source_symbol, size=size, source=None, destination="taxes"
        )
        time.sleep(0.05)
...
def process_trades(
    ...
    trigger_order_columns = [("trigger_name", "trigger_param", "trigger_order_id")]
    ...
    for (
        trigger_name_col,
        trigger_param_col,
        trigger_order_id_col,
    ) in trigger_order_columns:
        prev_triggered = {}
        ...
        for strategy_key in prev_buy_info.index:
            ...
            if trigger_order_id is None:
                ...
                prev_triggered[strategy_key] = (
                    trigger_order_id not in open_trigger_orders
                    or open_trigger_orders[trigger_order_id]["status"] != "open"
                )
        ...
        for strategy_key in prev_buy_info.index:
            ...
            if prev_triggered[strategy_key] == True:
                ...
                if trigger_name == "trailing":
                    end_time = time.time()
                    conditional_order_history = get_conditional_order_history(
                        client, end_time
                    )
                    ...
                    while trigger_order_id not in conditional_order_history["id"]:
                        ...
                        timestamps = get_timestamps(
                            conditional_order_history["createdAt"]
                        )
                    ...
                    assert (
                        conditional_order_history["id"] == trigger_order_i
                    ).sum() == 1
    ...
    for strategy_key in prev_buy_info.index:
        ...
        if len(triggered_for_strategy) == 1:
            ...
            prev_triggered[strategy_key] = reduce(
                lambda x, y: x or y, triggered_for_strategy
            )
    ...
    for strategy_key in buy_info.index:
        ...
        if ((prev_triggered[strategy_key] == True) and not changed) or triggered[
            strategy_key
        ]:
    ...
    for strategy_key in buy_info.index:
        if prev_triggered[strategy_key] == True:
            ...
            buy_info.loc[strategy_key, "usd_value"] = (
                prev_trigger_prices[strategy_key] * size
            )

            if buy_info.loc[strategy_key, "changed"]:
                sell_prices = append_to_dict_of_collections(
                    sell_prices,
                    prev_symbols[strategy_key],
                    prev_trigger_prices[strategy_key],
                )
                sell_sizes = append_to_dict_of_collections(
                    sell_sizes, prev_symbols[strategy_key], size
                )
    ...
    (
        ...
        total_balance,
        balances,
    ) = get_weight_diffs(client, buy_info, target_symbols)
    ...
    for symbol, weight_diff in weight_diffs[li_neg].items():
        if symbol != source_symbol and np.abs(weight_diff) > min_amount:
            ...
            size = (
                np.abs(weight_diff / actual_weights[symbol]) * balances["total"][symbol]
            )
    ...
    if taxes:
        ...
        for symbol in sell_prices.keys():
            ...
            for sell_price, sell_size in zip(sell_prices[symbol], sell_sizes[symbol]):
                ...
                if sell_value > buy_value:
                    ...
                    total_weight_of_subset = buy_info.loc[
                        strategy_subset, "weight"
                    ].sum()

                    for strategy_key in strategy_subset:
                        conditional_weight = (
                            buy_info.loc[strategy_key, "weight"]
                            / total_weight_of_subset
                        )
                        buy_info.loc[strategy_key, "usd_value"] -= (
                            conditional_weight * amount_to_taxes
                        )
        ...
        if total_amount_to_taxes > 0:
            transfer_taxes_to_taxes_subaccount(
                client, total_amount_to_taxes, debug=debug
            )
    ...
    for symbol, weight_diff in weight_diffs[li_pos].items():
        if symbol != source_symbol and weight_diff > min_amount:
            ...
            size, price = buy_assets(
                client, symbol, usd_size, round_n=5, debug=debug, return_price=True
            )
            print(f"Buy ({symbol}):\n\tsize: {size}\n\tprice: {price}")

            buy_history = append_to_dict_of_collections(
                buy_history, symbol, (price, size), collection_type="deque"
            )
...
def balance_portfolio(client, buy_info, debug=False, verbose=False):
    ...
    for strategy_key in buy_info.index:
        ...
        if trigger_order_id is None:
            ...
            if (
                not triggered[strategy_key]
                and buy_info.loc[strategy_key, "trigger_name"] == "trailing"
            ):
                trailing_trigger_prices[strategy_key] = open_trigger_orders[
                    trigger_order_id
                ]["triggerPrice"]
    ...
    for strategy_key, symbol in symbols.items():
        if symbol != source_symbol and trigger_order_ids[strategy_key] is not None:
            if buy_info.loc[strategy_key, "trigger_name"] == "trailing":
                size = min(
                    1.0, buy_info.loc[strategy_key, "weight"] / actual_weights[symbol]
                )

                if size * balances["usdValue"][symbol] / total_balance > min_amount:
                    size *= balances["total"][symbol]

                    trail_value = trailing_trigger_prices[strategy_key] - ftx_price(
                        client, symbol
                    )
    ...
    for strategy_key, symbol in symbols.items():
        if symbol != source_symbol and not triggered[strategy_key]:
            ...
            if (
                buy_info.loc[strategy_key, "trigger_name"] != "trailing"
                or trigger_order_ids[strategy_key] is None
            ):
                id, size = place_trigger_order(
                    client,
                    symbol,
...
def trading_pipeline(
    ...
    for strategy_key in strategies.keys():
        ...
        get_buys_and_sells_fns[strategy_key] = choose_get_buys_and_sells_fn(
            strategy_type
        )

    if os.path.exists("optim_results/displacements_and_last_signal_change_time.json"):
        with open(
            "optim_results/displacements_and_last_signal_change_time.json", "r"
        ) as file:
    ...
    try:
        while True:
            ...
            now_string = time.strftime("%d/%m/%Y %H:%M:%S", localtime)
            timeTo0 = time.mktime(
                datetime.strptime(now_string[:-6], "%d/%m/%Y %H").timetuple()
            )
            ...
            for displacement in displacement_values_unique:
                ...
                for key in keys:
                    ...
                    X = aggregate_F(
                        X_dict[coin], aggregate_N, from_minute=displacement > 0
                    )
                    ...
                    if buy:
                        if buy != prev_buy_state[key]:
                            ...
                            if initial_action_flag and last_buy_signal[key]:
                                last_signal_change_time = last_signal_change_times[
                                    key
                                ] + (time.time() - last_signal_change_times[key]) % (
                                    aggregate_N * 60 * 60
                                )
                                buy_info.loc[key, "buy_price"] = get_historical_price(
                                    client,
                                    get_symbol(coin, m, bear=False),
                                    last_signal_change_time,
                                )
                            else:
                                buy_info.loc[key, "buy_price"] = ftx_price(
                                    client, get_symbol(coin, m, bear=False)
                                )
                        ...
                        if (not sell) != prev_buy_state[key]:
                            ...
                            if initial_action_flag and last_buy_signal[key] == False:
                                ...
                                buy_info.loc[key, "buy_price"] = get_historical_price(
                                    client,
                                    get_symbol(coin, m_bear, bear=True),
                                    last_signal_change_time,
                                )
                            else:
                                buy_info.loc[key, "buy_price"] = ftx_price(
                                    client, get_symbol(coin, m_bear, bear=True)
                                )
...
if __name__ == "__main__":
    ...
    while error_flag:
        error_flag, ask_for_input, buy_info = trading_pipeline(
            buy_info=buy_info, ask_for_input=ask_for_input
        )
