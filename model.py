...
class RelationalMemory(nn.Module):
    ...
    def forward_step(self, inputs, memory, treat_input_as_matrix=False):
        ...
        if treat_input_as_matrix:
            ...
            inputs_reshape = inputs.view(inputs.shape[0], inputs.shape[1], -1)
            # apply linear layer for dim 2
            inputs_reshape = self.input_projector(inputs_reshape)
        else:
            # keep (Batch, ...) dim (0), flatten starting from dim 1
            inputs_reshape = inputs.view(inputs.shape[0], -1)
            # apply linear layer for dim 1
            inputs_reshape = self.input_projector(inputs_reshape)
            # unsqueeze the time step to dim 1
            inputs_reshape = inputs_reshape.unsqueeze(dim=1)
...
if __name__ == '__main__':
    ...
    # random input
    random_input = torch.randn((1, seq_length, input_size))
    # random targets
    # random_targets = torch.randn((1, seq_length, input_size))

    # take a one step forward
    logit, loss, next_memory = model(random_input, model_memory)

    import copy

    sd = copy.deepcopy(model.state_dict())

    print(sd['amount_layer.bias'], model.state_dict()['amount_layer.bias'])
    print()
