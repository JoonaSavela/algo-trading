...
def build_model_old(input_length = 4 * 14, alpha = 0.001):
...
def build_model(input_length = 4 * 14, alpha = 0.001):
    ...
    layer_2 = Dense(input_length * 3 // 2)(layer_1)
    layer_3 = Dense(input_length * 3 // 4)(layer_2)
    output_layer1 = Dense(3, activation='softmax')(layer_3)
    output_layer2 = Dense(1, activation='sigmoid')(layer_3)
    output_layer = concatenate([output_layer1, output_layer2])
    model = Model(input_layer, output_layer)
    model.compile(Adam(), 'mse', metrics=['accuracy'])
