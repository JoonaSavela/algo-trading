    ...
    while entry_i < N - 1:
        ...
        if not (buy_state or short):
            ...
            if trigger_name != 'trailing':
                ...
                # TODO: add random version of this
                accumulative_max = np.maximum.accumulate(np.concatenate([
                    np.array([max_price]),
                    sub_X[1:, 1] * price
                ]))
                max_price = accumulative_max[-2]
                trigger_prices1 = accumulative_max[:-1] - (1 - trigger_param) * buy_price
                trigger_prices2 = accumulative_max[1:] - (1 - trigger_param) * buy_price
...
# No commissions or spread
def get_trailing_stop_loss_wealths_from_sub_trades(sub_trade_wealths, min_sub_trade_wealths, max_sub_trade_wealths, trailing_stop_loss, total_months, return_total = True, return_as_log = False):
    ...
    for i in range(len(sub_trade_wealths)):
        ...
        # TODO: add a random version of this
        ...
        accumulative_max1 = accumulative_max[:-1]
        # for each candle/timestep, high occurs before low
        accumulative_max2 = accumulative_max[1:]

        li1 = min_sub_trade_wealths[i] < accumulative_max1 - trailing_diff
        li2 = min_sub_trade_wealths[i] < accumulative_max2 - trailing_diff
        ...
        if cond2:
            ...
            trade_wealths[i] = np.min([
                accumulative_max1[li1][0] - trailing_diff if cond1 else trade_wealths[i],
                (max(accumulative_max1[li2][0] - trailing_diff, min_sub_trade_wealths[i][li2][0]) + accumulative_max2[li2][0] - trailing_diff) / 2
            ])
